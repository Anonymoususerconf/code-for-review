...
# train models for commercial activeness prediction task
class Trainer_CAP(object):
    def __init__(self, args):
        super(Trainer_CAP, self).__init__()
        self.args = args
        self.root_path = '../'
    ...
    def data_prepare(self):
        poi_feature = np.load(self.root_path + 'data/{task}/poi_features.npy'.format(task=self.args.task))
        img_feature = np.load(self.root_path + 'data/{task}/img_features.npy'.format(task=self.args.task))
        label = np.load(self.root_path + 'data/{task}/label_array.npy'.format(task=self.args.task))
        poi_feature = torch.from_numpy(poi_feature[:, 15:]).float().to('cuda')
        img_feature = torch.from_numpy(img_feature).float().to('cuda')
        label = torch.from_numpy(label).float().to('cuda')
        return poi_feature, img_feature, label
    ...
    def train(self, train_id, val_id, test_id, save_model, log):
        poi_feature, img_feature, label = self.data_prepare()
        g = self.load_graph()
        ...
        for epoch in range(self.args.epoch_num+1):
            ########################## train ###########################
            model.train()
            pred = model(poi_feature, img_feature).squeeze(1)
            ...
            if rmse_val < best_rmse and epoch > self.args.epoch_num * 0.5:
